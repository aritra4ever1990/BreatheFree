...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const closeAchModal = $('closeAchModal');
    ...
    // Storage helpers
    function loadEvents(){ try{ const r=localStorage.getItem(EVENTS_KEY); return r?JSON.parse(r):[] }catch(e){return[]} }
    function saveEvents(e){ localStorage.setItem(EVENTS_KEY, JSON.stringify(e)); }

    function loadSettings(){ try{ const r=localStorage.getItem(SETTINGS_KEY); if(!r) return { price:0.5, dailyTarget:10, currency:'$', cravingDuration:5, streakTarget:7 }; return Object.assign({ price:0.5, dailyTarget:10, currency:'$', cravingDuration:5, streakTarget:7 }, JSON.parse(r)) }catch(e){return { price:0.5, dailyTarget:10, currency:'$', cravingDuration:5, streakTarget:7 }}}
    function saveSettings(s){ localStorage.setItem(SETTINGS_KEY, JSON.stringify(s)); }

    function loadAchievements(){ try{ const r=localStorage.getItem(ACH_KEY); return r?JSON.parse(r):[] }catch(e){return[]} }
    function saveAchievements(a){ localStorage.setItem(ACH_KEY, JSON.stringify(a)); }

    function loadCraving(){ try{ const r=localStorage.getItem(CRAVING_KEY); return r?JSON.parse(r):{ endTs:null } }catch(e){return { endTs:null }}}
    function saveCraving(c){ localStorage.setItem(CRAVING_KEY, JSON.stringify(c)); }

    function pushUndo(p){ localStorage.setItem(UNDO_KEY, JSON.stringify({ payload:p, ts:new Date().toISOString() })); if(undoBtn) undoBtn.disabled=false; }
    function popUndo(){ const r=localStorage.getItem(UNDO_KEY); if(!r) return null; localStorage.removeItem(UNDO_KEY); if(undoBtn) undoBtn.disabled=true; return JSON.parse(r).payload; }
    ...
    function showToast(message, { timeout=5000, actionText=null, action=null } = {}){
      ...
      if(actionText && action){
        ...
        btn.addEventListener('click', (e)=> { action(); remove(); });
      ...
      let dismissed=false;
      function remove(){ if(dismissed) return; dismissed=true; t.style.opacity=0; setTimeout(()=>{ try{ toastContainer.removeChild(t);}catch(e){} },220) }
    ...
    function addEvent(ts = new Date().toISOString()){
      ...
      const events = loadEvents(); events.push({ id:uid(), ts }); events.sort((a,b)=> new Date(b.ts)-new Date(a.ts)); saveEvents(events); render();
    }
    ...
    // Craving timer logic (duration uses settings)
    let lastCravingTimerInterval = null;
    function cravingDurationMs(){
      const s = loadSettings(); return Math.max(1, parseFloat(s.cravingDuration) || 5) * 60 * 1000;
    }
    ...
    function startCraving(){
      ...
      lastCravingTimerInterval = setInterval(()=>{ updateCravingUI(); if(!isCravingActive()){ clearInterval(lastCravingTimerInterval); lastCravingTimerInterval=null; completeCraving(); } }, 1000);
      showToast('Craving started', { timeout:2500 });
    }
    function stopCraving(){
      saveCraving({ endTs: null }); if(lastCravingTimerInterval){ clearInterval(lastCravingTimerInterval); lastCravingTimerInterval = null; }
      updateCravingUI(); showToast('Craving stopped', { timeout:2000 });
    }

    function completeCraving(){
      // add achievement record with metadata placeholders
      ...
      // Confetti + non-blocking toast with action to add details
      startConfetti();
    ...
    function closeAchModalFunc(){ achModal.classList.add('hidden'); achModal.setAttribute('aria-hidden','true'); activeAchievementId = null; }
    ...
    function renderAchievements(){
      ...
      if(!ach.length){ const li=document.createElement('li'); li.textContent='No achievements yet — complete a 5-min craving to earn one.'; awardsList.appendChild(li); return; }
      ach.slice(0,50).forEach(a=>{
        const li=document.createElement('li'); li.textContent = `${a.ts.slice(0,10)} — ${new Date(a.ts).toLocaleTimeString()}` + (a.reason?` • ${a.reason}`:'') + (a.location?` • ${a.location}`:'');
    ...
    function startConfetti(){
      ...
      for(let i=0;i<num;i++){ pieces.push({ x: Math.random()*cv.width, y: Math.random()*-cv.height*0.5, w:8+Math.random()*10, h:8+Math.random()*10, color: colors[Math.floor(Math.random()*colors.length)], rot: Math.random()*Math.PI*2, velX:(Math.random()-0.5)*4, velY:2+Math.random()*6, spin:(Math.random()-0.5)*0.2 }) }
      let t0=null; const duration=2200;
      function step(ts){ if(!t0) t0=ts; const elapsed=ts-t0; ctx.clearRect(0,0,cv.width,cv.height);
        ...
        if(elapsed<duration) requestAnimationFrame(step); else { let fade=1; const fadeStep=()=>{ fade-=0.06; ctx.fillStyle=`rgba(255,255,255,${1-fade})`; ctx.fillRect(0,0,cv.width,cv.height); if(fade>0) requestAnimationFrame(fadeStep); else { confettiActive=false; try{ document.body.removeChild(cv);}catch(e){} } }; requestAnimationFrame(fadeStep); } }
      requestAnimationFrame(step);
      const onResize=()=>{ cv.width=window.innerWidth; cv.height=window.innerHeight; }; window.addEventListener('resize', onResize, { once:false });
      setTimeout(()=>window.removeEventListener('resize', onResize), duration+600);
    }

    // Analytics: counts & sparkline, streaks
    function isoDate(d=new Date()){ return d.toISOString().slice(0,10); }
    function countsLastNDays(n){
      const events=loadEvents(); const counts={}; for(let i=0;i<n;i++){ const day=isoDate(new Date(Date.now()-i*24*3600*1000)); counts[day]=0; } events.forEach(e=>{ const d=e.ts.slice(0,10); if(d in counts) counts[d]++; }); return counts;
    }
    ...
    function updateStatsAndCharts(){
      ...
      if(weeklyCanvas){
        ...
        if(!weeklyChart){ weeklyChart = new Chart(weeklyCanvas.getContext('2d'), { type:'bar', data:{ labels:labels7, datasets:[ { label:'Cigarettes', data:data7, backgroundColor:'rgba(0,168,255,0.85)' }, { label:'Target', data:targetData, type:'line', borderColor:'rgba(255,125,85,0.95)', borderWidth:2, pointRadius:0, fill:false } ] }, options:{ responsive:true, plugins:{ legend:{ display:false } }, scales:{ y:{ beginAtZero:true } } } }); } else { weeklyChart.data.labels=labels7; weeklyChart.data.datasets[0].data=data7; weeklyChart.data.datasets[1].data=targetData; weeklyChart.update(); }
      }
      ...
      if(monthlyCanvas){
        ...
        if(!monthlyChart){ monthlyChart = new Chart(monthlyCanvas.getContext('2d'), { type:'line', data:{ labels:labels30, datasets:[{ label:'Cigarettes', data:data30, borderColor:'rgba(0,168,255,0.9)', backgroundColor:'rgba(0,168,255,0.08)', fill:true, pointRadius:1 }] }, options:{ responsive:true, plugins:{ legend:{ display:false } }, scales:{ x:{ display:false }, y:{ beginAtZero:true } } } }); } else { monthlyChart.data.labels=labels30; monthlyChart.data.datasets[0].data=data30; monthlyChart.update(); }
      }

      // sparkline for achievements per day over last 7 days
      if(sparklineCanvas){
        const ach = loadAchievements();
        ...
        if(!sparkChart){ sparkChart = new Chart(sparklineCanvas.getContext('2d'), { type:'line', data:{ labels, datasets:[{ data, borderColor:'rgba(123,97,255,0.9)', backgroundColor:'rgba(123,97,255,0.08)', fill:true, pointRadius:0 }] }, options:{ responsive:true, plugins:{ legend:{ display:false } }, scales:{ x:{ display:false }, y:{ display:false } }, elements:{ line:{ tension:0.4 } } } }); } else { sparkChart.data.labels=labels; sparkChart.data.datasets[0].data=data; sparkChart.update(); }
      }
    }
    ...
    function computeStreaks(){
      ...
      let cur=0; let day = new Date(); while(true){ const dstr = isoDate(day); if(set.has(dstr)){ cur++; day.setDate(day.getDate()-1); } else break; }
      // compute best streak by scanning unique dates
      let best=0; let streak=0; let prev=null;
      const uniq = dates.map(d=> new Date(d));
      for(let i=0;i<uniq.length;i++){
        if(i===0){ streak=1; prev=uniq[0]; }
        else { const diffDays = Math.round((uniq[i]-prev)/(24*3600*1000)); if(diffDays===1){ streak++; prev=uniq[i]; } else { best = Math.max(best, streak); streak=1; prev=uniq[i]; } }
      }
    ...
    function renderAnalytics(){
      ...
      const weekCount = ach.filter(a=> { const d=new Date(a.ts); const diffDays = Math.floor((new Date().setHours(0,0,0,0) - new Date(d.getFullYear(), d.getMonth(), d.getDate()).getTime())/(24*3600*1000)); return diffDays>=0 && diffDays<7; }).length;
    ...
    // render achievements list etc.
    function renderAchievements(){ const ach=loadAchievements(); if(achCount) achCount.textContent=ach.length; if(!awardsList) return; awardsList.innerHTML=''; if(!ach.length){ const li=document.createElement('li'); li.textContent='No achievements yet — complete a craving to earn one.'; awardsList.appendChild(li); return; } ach.slice(0,50).forEach(a=>{ const li=document.createElement('li'); li.textContent=`${a.ts.slice(0,10)} • ${new Date(a.ts).toLocaleTimeString()}${a.reason?` • ${a.reason}`:''}${a.location?` • ${a.location}`:''}`; const edit=document.createElement('button'); edit.className='btn outline'; edit.textContent='Edit'; edit.style.marginLeft='8px'; edit.addEventListener('click', ()=> openAchModal(a.id)); li.appendChild(edit); awardsList.appendChild(li); }); }

    // Ach modal handlers
    function openAchModal(id){ const a = loadAchievements().find(x=>x.id===id); if(!a) return; achReasonInput.value=a.reason||''; achLocationInput.value=a.location||''; achModal.classList.remove('hidden'); achModal.setAttribute('aria-hidden','false'); activeAchievementId=id; }
    function closeAchModal(){ achModal.classList.add('hidden'); achModal.setAttribute('aria-hidden','true'); activeAchievementId=null; }
    function saveAchModal(){ if(!activeAchievementId) return closeAchModal(); const arr=loadAchievements(); const a=arr.find(x=>x.id===activeAchievementId); if(!a) return closeAchModal(); a.reason=achReasonInput.value.trim(); a.location=achLocationInput.value.trim(); saveAchievements(arr); closeAchModal(); renderAchievements(); renderAnalytics(); showToast('Achievement details saved', { timeout:2200 }); }

    // update UI aspects and charts
    function updateCravingUI(){
      const active = isCravingActive();
      if(cravingTimerEl){ if(active) cravingTimerEl.textContent = 'Remaining: ' + formatMMSS(remainingMs()); else cravingTimerEl.textContent = ''; }
    ...
    function render(){
      ...
      renderEventsList(); renderAchievements(); renderAnalytics(); updateStatsAndCharts(); updateCravingUI(); if(undoBtn) undoBtn.disabled = !localStorage.getItem(UNDO_KEY);
    }
    ...
    if(decrementBtn) decrementBtn.addEventListener('click', ()=>{
      const events = loadEvents(); const date = dateInput?dateInput.value||isoDate():isoDate(); const match = events.find(e=>e.ts.slice(0,10)===date); if(!match) return alert('No events on selected date to remove.'); if(!confirm('Remove the most recent event for this date?')) return; removeEvent(match.id);
    });
    ...
    if(undoBtn) undoBtn.addEventListener('click', ()=>{ const payload = popUndo(); if(!payload) return showToast('Nothing to undo',{}); if(payload.action==='delete'){ const events=loadEvents(); events.push(payload.event); events.sort((a,b)=>new Date(b.ts)-new Date(a.ts)); saveEvents(events); render(); } else if(payload.action==='clear'){ saveEvents(payload.events||[]); render(); } });
    ...
    if(openSettingsBtn) openSettingsBtn.addEventListener('click', ()=> {
      const s = loadSettings(); if(cigPriceInput) cigPriceInput.value = s.price; if(dailyTargetInput) dailyTargetInput.value = s.dailyTarget; if(currencyInput) currencyInput.value = s.currency; if(cravingDurationInput) cravingDurationInput.value = s.cravingDuration; if(streakTargetInput) streakTargetInput.value = s.streakTarget;
    ...
    if(resetSettingsBtn) resetSettingsBtn.addEventListener('click', ()=> { if(!confirm('Reset settings to defaults?')) return; saveSettings({ price:0.5, dailyTarget:10, currency:'$', cravingDuration:5, streakTarget:7 }); render(); });

    // Ach modal
    if(closeAchModal) closeAchModal.addEventListener('click', ()=> closeAchModal());
    if(cancelAchBtn) cancelAchBtn.addEventListener('click', ()=> closeAchModal());
    if(saveAchBtn) saveAchBtn.addEventListener('click', ()=> saveAchModal());

    // keyboard shortcuts
    window.addEventListener('keydown', (e)=> { if((e.key==='N'||e.key==='n') && document.activeElement.tagName!=='INPUT') addEvent(new Date().toISOString()); if((e.key==='U'||e.key==='u') && (!undoBtn||!undoBtn.disabled)) undoBtn&&undoBtn.click(); if((e.key==='E'||e.key==='e') && document.activeElement.tagName!=='INPUT') exportBtn&&exportBtn.click(); });
    ...
    // helper functions declared above need to be hoisted; small re-declarations:
    function updateCravingUI(){ const active=isCravingActive(); if(cravingTimerEl){ if(active) cravingTimerEl.textContent='Remaining: '+formatMMSS(remainingMs()); else cravingTimerEl.textContent=''; } [addNowBtn, incrementBtn, addAtBtn].forEach(el=>{ if(el) el.disabled=active; }); if(startCravingBtn) startCravingBtn.disabled=active; if(stopCravingBtn) stopCravingBtn.disabled=!active; }
    function loadAchievements(){ try{ const r=localStorage.getItem(ACH_KEY); return r?JSON.parse(r):[] }catch(e){return[]} }
    function saveAchievements(a){ localStorage.setItem(ACH_KEY, JSON.stringify(a)); }
    function loadCraving(){ try{ const r=localStorage.getItem(CRAVING_KEY); return r?JSON.parse(r):{ endTs:null }}catch(e){return { endTs:null }} }
    function saveCraving(c){ localStorage.setItem(CRAVING_KEY, JSON.stringify(c)) }
    function showToast(...args){ return (window.showToastOriginal||showToastInternal)(...args); }
    function showToastInternal(message, opts){ return (function(){ if(!toastContainer) return null; const t=document.createElement('div'); t.className='toast'; const msg=document.createElement('div'); msg.className='msg'; msg.textContent=message; const actions=document.createElement('div'); actions.className='actions'; if(opts && opts.actionText && opts.action){ const btn=document.createElement('button'); btn.className='btn-small-primary'; btn.textContent=opts.actionText; btn.addEventListener('click', ()=>{ opts.action(); remove(); }); actions.appendChild(btn); } const closeBtn=document.createElement('button'); closeBtn.className='btn-small-outline'; closeBtn.textContent='Close'; closeBtn.addEventListener('click', remove); actions.appendChild(closeBtn); t.appendChild(msg); t.appendChild(actions); toastContainer.prepend(t); let dismissed=false; function remove(){ if(dismissed) return; dismissed=true; t.style.opacity=0; setTimeout(()=>{ try{ toastContainer.removeChild(t); }catch(e){} },220); } setTimeout(remove, (opts&&opts.timeout)||5000); return t; })(); }
    window.showToastOriginal = showToastInternal;

    // expose some debug helpers if needed
    window._breathefree = { addEvent, loadEvents, loadSettings, loadAchievements, startCraving, stopCraving, showToast: showToastInternal };
